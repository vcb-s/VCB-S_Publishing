...
1. **BANGUMI.MOE**（负责推送到：SHARE.DMHY.ORG / ACG.RIP / ACGNX.SE / SHARE.ACGNX.SE）
2. **NYAA.SI**

若 **BANGUMI.MOE** 无法发布，则需**手动发布其原本推送的所有站点**，此时需在以下 **4 个站点手动发布**：

- **SHARE.DMHY.ORG**
- **ACG.RIP**
- **ACGNX.SE**
- **NYAA.SI**
>**SHARE.ACGNX.SE** 会自动从其他站点同步，不需要进行手动发布

若发布任务中有站点无法使用，请：

1. **记录当前任务以及不可用站点**；
2. 待站点恢复后**完成补发**；
3. 将补发后获得的链接**更新到主站对应帖子**中。
...
### 如果 BANGUMI.MOE 网站无法使用

需要**手动发布**原本由 BANGUMI.MOE 推送的内容到以下 **3 个站点**：

- **SHARE.DMHY.ORG**
- **ACG.RIP**
- **ACGNX.SE**

其中 **SHARE.ACGNX.SE** 会自动同步 **SHARE.DMHY.ORG** 等站点的内容，**无需手动发布**；但仍请**务必检查**并确认同步成功。

发布顺序需特别注意：**ACGNX.SE 必须在 NYAA.SI 发布之前完成发布**。
**NYAA.SI** 按原流程**照常发布**。

#### 发布稿与格式要求：

- **SHARE.DMHY.ORG** 与 **ACGNX.SE**：使用与 **BANGUMI.MOE** 相同的**标题**与**发布稿正文**。
- **ACG.RIP** 正文为 **bbcode**，需使用 [html 转换器](https://github.com/vcb-s/Html-Converter) 将 HTML 发布稿转换为 bbcode。
- **类别**的对应关系请参照如下表格：

| BANGUMI.MOE | DMHY.ORG | ACG.RIP |
|-------------|----------|---------|
| 合集          | 季度全集       | 动画      |
| 剧场版         | 動畫     | 动画      |
| 动画          | 動畫       | 动画      |
...
|--------------------------------------|--------------|
| Anime - Raw                          | A.RAW        |
| Anime - English-translated           | A.E.T.       |
| Anime - Non-English-translated       | A.N.E.T.     |
| Live Action - Raw                    | L.A.RAW      |
| Live Action - English-translated     | L.A.E.T.     |
| Live Action - Non-English-translated | L.A.N.E.T    |

发布完成后，应获得以下 **4 个站点**各自对应的 URL：

- SHARE.DMHY.ORG（手动发布生成）
- ACG.RIP（手动发布生成）
- ACGNX.SE（手动发布生成）
- SHARE.ACGNX.SE（由同步生成；请检查确认同步成功后记录其 URL）
...
发布到主站时，链接必须严格按照以上顺序排列，顺序不得改变。如某个站点链接不可用/无法获取，则直接跳过该站点链接，保留其余链接的相对顺序不变。
...
<!-- 这里空行，然后下一行开始点击 `link` 粘贴刚才发布公网各站的链接，注意勾选 `在新标签页中打开链接`。链接必须严格按照以下顺序排列，顺序不得改变。如某个站点链接不可用/无法获取，则直接跳过该站点链接，保留其余链接的相对顺序不变。 -->
